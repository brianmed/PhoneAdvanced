...
sub _build_dbh {
    ...
    return DBI->connect_cached($self->dbdsn(), "kevin", "the_trinity", { RaiseError => 1, PrintError => 0, AutoCommit => 1 });
}
...
sub last_insert_id
{
    ...
    return($self->dbh()->last_insert_id($catalog, $schema, $table, $field, $attrs));
}
